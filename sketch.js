...
function preload(){
  ...
  balaimg = loadImage("assets/bala.webp")
...
console.log(zombieGroup[i].positionX);
 for(var i=0;i<zombieGroup.length;i++){     
      
  if(zombieGroup[i].isTouching(player) || zombieGroup[i].y===1000){
       zombieGroup[i].destroy()
       life= life-1;
       } 
...
//si los zombies se escapan pierdes una vida
...
function enemy(){
  if (frameCount%100===0){
    ...
    zombie= createSprite(1400,600, 40,80);
    //condicion para aumentar la velocidad 
    zombie.velocityX = -(2+2*score/5)
    zombie.y= Math.round(random(500,900))
